...
const Occupational = ({ onSave }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.occupational) {
            setPatientComplaint(data.occupational.patientComplaint || '');
            setEarlierTherapies(data.occupational.earlierTherapies || '');
            setAssessmentPoints(data.occupational.assessmentPoints || '');
            setTestsAdministered(data.occupational.testsAdministered || '');
            setProvisionalDiagnosis(data.occupational.provisionalDiagnosis || '');
            setManagementPlan(data.occupational.managementPlan || '');
            setReferrals(data.occupational.referrals || '');
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation
    if (!patientComplaint || !earlierTherapies || !assessmentPoints || !testsAdministered || !provisionalDiagnosis || !managementPlan || !referrals) {
